...
class SSMClassifier(nn.Module):
    def __init__(self, ssm_state_dim=256, ssm_out_dim=256, n_classes=10):
        super().__init__()
        ...
        self.ssm = SSM(input_dim=128, state_dim=ssm_state_dim, output_dim=ssm_out_dim)
        self.pool = nn.AdaptiveAvgPool2d((32, 1))
...
def evaluate_child(model, child, test_loader, use_mask=False):
    ...
    with torch.no_grad():
        for x, y in tqdm(test_loader, desc="Testing Child SSM Model", leave=False):
            ...
            state = model.ssm.h0.unsqueeze(0).expand(Bz, -1).contiguous()
            outputs = []
...
if __name__ == "__main__":
    ...
    parser.add_argument("--checkpoint", type=str, default="best_cifar_ssm.pth")
    ...
    A_diag = model.ssm.A_diag.data.clone()
    # print(A.shape, A)
    B = model.ssm.B.data.clone()
    C = model.ssm.C.data.clone()
    D = model.ssm.D.data.clone()

    desired_quantized_model = f"quantized_ssm_structured_cifar10.pth"
    ...
    if not os.path.exists(trained_unpruned_child_model):
        ...
        for epoch in range(num_epochs):
            ...
            for x, y in tqdm(train_loader, desc=f"Child SSM Training Epoch {epoch+1}/{num_epochs}", leave=False):
                ...
                state = model.ssm.h0.unsqueeze(0).expand(Bz, -1).contiguous()
                parent_state = model.ssm.h0.unsqueeze(0).expand(Bz, -1).contiguous()
                outputs = []
                ...
                for t in range(L):
                    ...
                    state, y_t = child(state, ut)
                    with torch.no_grad():
                        parent_state, parent_y_t = model.ssm.single_step(parent_state, ut)
        ...
        for epoch in range(num_epochs_prune):
            ...
            if current_acc < target_acc and (epoch+1) < num_epochs_prune:
                ...
                with torch.no_grad():
                    ...
                    child.D.adapter._threshold -= 0.001
            else:
                lambda_sparse = 1e-8
    ...
    with torch.no_grad():
        for x, y in tqdm(test_loader, desc="Collecting Statistics", leave=False):
            ...
            for t in range(L):
                ...
                model_state = model.ssm.hidden_update(model_state, ut)                
                ...
                model_output = model.ssm.get_output(model_state, ut)
                running_stats[t].update(model_output)

                outputs.append(y_t)
                
            output = torch.stack(outputs, dim=1)
            running_stats_y.update(output)
            output = model.ssm.activation(output)
            output = model.ssm.norm(output)
            output = output.transpose(1, 2).contiguous()
            logits = model.fc(output)
    ...
    for error_rate in error_rates:
        ...
        #                 ut = u_t[t]
        #                 model_state = model.ssm.hidden_update(model_state, ut, inject_type="output", error_rate=error_rate)

        #                 y_t = model.ssm.get_output(model_state, ut)
        #                 outputs.append(y_t)
    ...
    output_path = f"cifar10_stats_output_zeroing.json"
    ...
    for error_rate in error_rates:
        ...
        #                 ut = u_t[t]
        #                 model_state = model.ssm.hidden_update(model_state, ut, inject_type="weight", error_rate=error_rate)
    ...
    output_path = f"cifar10_stats_weight_zeroing.json"
    with open(output_path, "w") as f:
        json.dump(data, f, indent=4)
